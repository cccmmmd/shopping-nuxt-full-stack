...
const uploadImage = async () => {
	...
	try {
		const { data, error } = await supabase.storage
			.from("images")
			.upload(`public/${imgtemp.name}`, imgtemp, {
				cacheControl: "3600",
				upsert: false,
			});
		if (data) {
			...
			const { data } = await supabase.storage
				.from("images")
				.getPublicUrl(`public/${imgtemp.name}`);
	...
	<form class="max-w-xl mx-auto mb-11">
		...
		<div class="mb-5 mb-10">
			...
			<div
				...
				id="user_avatar_help"
			>
				A gooe product image let you sell more !
			</div>
			<br />
			<button
				@click="uploadImage"
				type="button"
				class="text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-blue-800 me-2 mb-2"
			>
				Upload image
			</button>
			<!-- success -->
			...
			<div v-if="imageerror">
				...
				>
					<span class="font-medium">Oh, snapp!</span> {{ imageerror }}
				</p>
			</div>
		</div>
